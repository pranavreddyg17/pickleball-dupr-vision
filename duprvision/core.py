...
def init_db():
    from .competition import SCHEMA as competition_schema
    ...
    with connect() as db:
        ...
        CREATE TABLE IF NOT EXISTS playing_events (
          id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
          kind TEXT NOT NULL, location TEXT NOT NULL, address TEXT NOT NULL DEFAULT '',
          starts_at TEXT NOT NULL, ends_at TEXT NOT NULL, timezone TEXT NOT NULL,
          visibility TEXT NOT NULL CHECK(visibility IN ('followers','private')),
          note TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS playing_events_user_time ON playing_events(user_id, starts_at, ends_at);
        CREATE TABLE IF NOT EXISTS playing_event_places (
        ...
        """)
        db.executescript(competition_schema)
        db.execute("BEGIN IMMEDIATE")
...
def score_history(db, user_id):
    ...
    rows = db.execute("""SELECT videos.created_at, analyses.result_json,k.cache_key FROM analyses
      JOIN videos ON videos.id=analyses.video_id
      LEFT JOIN review_cache_keys k ON k.video_id=videos.id
      WHERE videos.user_id=? AND videos.status='COMPLETED' ORDER BY videos.created_at""", (user_id,)).fetchall()
    reports = [(row, json.loads(row['result_json'])) for row in rows]
    version = ('vision_score_v2' if any(r.get('performance', {}).get('version') == 'vision_score_v2'
                                      for _, r in reports) else 'vision_score_v1')
    days, seen = {}, set()
    earlier = 0
    for row, result in reports:
        ...
        same_version = performance.get('version', 'vision_score_v1') == version
        score = performance.get('score') if same_version else None
        if not same_version and performance.get('score') is not None:
            earlier += 1
        if score is not None and row['cache_key']:
            if row['cache_key'] in seen:
                score = None
            else:
                seen.add(row['cache_key'])
        days.setdefault(day, []).append(score)
    daily = []
    ...
    return {"days": daily, "average": average, "scored_days":len(rated_days),
            "clips": sum(d["clips"] for d in daily), "streak": streak,
            "rated_clips":sum(d["rated_clips"] for d in daily),
            "today": today.isoformat(), "timezone": str(zone), "kind": version, "earlier_reports":earlier}
